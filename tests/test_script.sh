...
echo "modified_content" >> "$MOUNT_DIR/base.txt" 2>&1 
if [ $(grep -c "modified_content" "$UPPER_DIR/base.txt" 2>/dev/null) -eq 1 ] && [ $(grep -c "modified_content" "$LOWER_DIR/base.txt" 2>/dev/null) -eq 0 ]; then 
    echo -e "${GREEN}PASSED${NC}" 
else 
    echo -e "${RED}FAILED${NC}" 
fi 
